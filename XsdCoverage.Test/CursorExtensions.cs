...
namespace XsdCoverage
{
	public static class CursorExtensions
	{
		...
		{
			return new CursorList<T>(cursor, cursor.NameSpace + localName);
		}

		public static T Element<T>(this ICursor<XElement> cursor, string localName) where T : ICursor<XElement>
		{
			XElement resultTarget = cursor.Target.Element(cursor.NameSpace + localName);
			if (resultTarget == null)
			{
				if (!cursor.Build)
				{
					...
					throw new FormatException(string.Format("element `{0}' not found in {1}",
						cursor.NameSpace + localName, cursor.Target.GetAbsoluteXPath()));
				}
				resultTarget = new XElement(cursor.NameSpace + localName);
